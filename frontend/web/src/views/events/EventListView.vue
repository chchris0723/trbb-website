      ...
      <div class="container nav-inner">
        ...
        <div class="flex items-center gap-2">
          ...
          <template v-if="auth.isLoggedIn">
            <RouterLink to="/me" class="btn btn-ghost btn-sm">會員中心</RouterLink>
          </template>
          <template v-else>
...
<style scoped>
.navbar { position:fixed;top:0;left:0;right:0;z-index:100;background:rgba(0,0,0,0.9);backdrop-filter:blur(12px);border-bottom:1px solid var(--color-border); }
.nav-inner { display:flex;align-items:center;justify-content:space-between;height:64px; }
.nav-logo { font-size:2rem; }
.nav-link { font-family:var(--font-cond);font-weight:600;letter-spacing:.05em;text-transform:uppercase;font-size:.85rem;color:var(--color-gray-1);transition:color .2s; }
