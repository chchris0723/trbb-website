    ...
    <div class="auth-box wide">
      ...
      <div class="steps">
        <div class="step" :class="{ active: step >= 1, done: step > 1 }">
          <span class="step-num">1</span><span class="step-label">基本資料</span>
        </div>
      ...
      <template v-if="step === 1">
        <div class="form-row">
          <div class="form-group">
            ...
            <input v-model="form.username" type="text" placeholder="英數字，3~50 字元" />
            <p class="hint">登入用的帳號 ID，建立後不可更改</p>
          </div>
          <div class="form-group">
            <label>姓名 <span class="req">*</span></label>
            <input v-model="form.display_name" type="text" placeholder="真實姓名" />
          </div>
        </div>
        <div class="form-row">
          <div class="form-group">
...
const form    = ref({
  username: '', display_name: '', email: '', phone: '', password: '', confirm_password: ''
})

async function handleSubmit() {
  error.value = ''
  const { username, display_name, email, phone, password, confirm_password } = form.value
  ...
  loading.value = true
  try {
    await api.post('/auth/register', { username, display_name, email, phone, password })
    step.value = 2
  } catch(e) {
...
.auth-page {
  ...
  padding: 2rem 1rem; position: relative; background: var(--color-bg);
}
...
.auth-title { font-family: var(--font-cond); font-size: 1.1rem; letter-spacing: 0.1em; text-align: center; color: var(--color-gray-2); text-transform: uppercase; margin-bottom: 1.5rem; }
...
.step-num {
  ...
  color: var(--color-gray-2); transition: all 0.3s;
}
...
.step-label { font-size: 0.72rem; color: var(--color-gray-2); }
.step.active .step-label { color: var(--color-white); }
.step-line { flex: 1; height: 2px; background: var(--color-border); margin: 0 0.75rem; min-width: 60px; margin-bottom: 1rem; transition: background 0.3s; }
...
.hint { font-size: 0.72rem; color: var(--color-gray-2); margin-top: 0.25rem; }
...
.pending-box h2 { font-size: 1.5rem; margin-bottom: 0.75rem; }
.pending-box p { color: var(--color-gray-1); font-size: 0.9rem; line-height: 1.7; margin-bottom: 0.5rem; }
